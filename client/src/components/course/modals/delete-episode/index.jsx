...
import { remove as removeEpisode } from 'src/services/episodes';
...
export default function DeleteEpisodeModal() {
    ...
    function remove({ setIsLoading }) {
        ...
        removeEpisode(courseId, selectedEpisode.id).then(() => {
            setIsLoading(false);
    ...
    return (
        <DeleteModal
            modals={modals}
            setModals={setModals}
            description="When you delete a episode you lose all the information about it, likes, tests."
            remove={remove}
        />
    );
}
